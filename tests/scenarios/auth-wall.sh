...
rc=0
out1="$(timeout 60 node "$CLI_JS" navigate https://github.com/settings 2>&1)" || rc=$?
...
rc=0
out2="$(timeout 60 node "$CLI_JS" navigate https://github.com/login 2>&1)" || rc=$?
[[ "$rc" == "0" || "$rc" != "77" ]] || {
...
# ── 3. screenshot to auth-gated URL → exit 77 ───────────────────────────────
rc=0
out3="$(timeout 60 node "$CLI_JS" screenshot https://github.com/settings --out=/dev/null 2>&1)" || rc=$?
