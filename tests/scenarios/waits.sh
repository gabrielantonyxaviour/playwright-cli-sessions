...
rc=0
out1="$(timeout 60 node "$CLI_JS" navigate "$URL" '--wait-for-text=Example' 2>&1)" || rc=$?
...
rc=0
out2="$(timeout 60 node "$CLI_JS" navigate "$URL" '--wait-for-text=NeverAppears_xyz_12345' 2>&1)" || rc=$?
...
rc=0
out3="$(timeout 60 node "$CLI_JS" navigate "$URL" '--wait-for-count=p:1' 2>&1)" || rc=$?
...
rc=0
out4="$(timeout 60 node "$CLI_JS" navigate "$URL" '--wait-for-count=p:9999' 2>&1)" || rc=$?
...
rc=0
out5="$(timeout 60 node "$CLI_JS" navigate "$URL" --wait-for-network=idle 2>&1)" || rc=$?
...
rc=0
out6="$(timeout 60 node "$CLI_JS" navigate "$URL" '--wait-for-text=Example' '--wait-for=h1' 2>&1)" || rc=$?
